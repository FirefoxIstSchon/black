	...
	public UserModel elicitRank(Bid bid, UserModel userModel){
		...
		for(int i=0; i<= currentOrder.size()-1; i++){
			...
			if(comparResult == 1) {  // bid < iBid
				currentOrder.add(i,bid);
				BidRanking newRanking = new BidRanking(currentOrder, currentRanking.getLowUtility(), currentRanking.getHighUtility());
				UserModel newModel = new UserModel(newRanking);
				return newModel;
			}
		}
